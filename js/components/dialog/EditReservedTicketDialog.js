...
<div class="mdc-dialog">
    <div class="mdc-dialog__container" >
        ...
        <h2 class="mdc-dialog__title d-flex align-center pt-2">
            ...
            <span>{{#if reservedTicket.is_purchased}}View Reserved Ticket{{else}}Edit Reserved Ticket{{/if}}</span>
        </h2>
        <div class="mdc-dialog__content">
            <form autocomplete="off" class="my-2">
                {{#if reservedTicket.is_purchased}}
                    <div class="d-flex align-center text-green pb-4">
                        <i class="material-icons icon-medium vertical-align-middle mr-4">local_activity</i>
                        <h3 class="font-weight-normal my-0">Reserved ticket has been sold</h3>
                    </div>
                {{/if}}
                <label class="d-block">Event</label>
                <h2 class="my-0">{{eventName}}</h2>
                <label class="d-block mt-2">Ticket</label>
                <div class="bg-grey-lighten-3 rounded-sm w-100 mb-4" style="box-shadow:0 0 0 1px #BDBDBD inset">
                ...
                <div class="page-main overflow-y-auto"{{#unless reservedTicket.is_purchased}} style="height:27rem"{{/unless}}>
                    {{#if reservedTicket.is_purchased}}
                        ...
                        <div class="bg-grey-lighten-3 rounded-sm w-100" style="box-shadow:0 0 0 1px #BDBDBD inset">
                            <div class="d-flex flex-column flex-grow-1 flex-shrink-1 text-left px-4 py-2" style="min-width:5rem">
                                <h3 class="text-truncate my-0">{{reservedTicket.user.display_name}}</h3>
                                <div class="text-truncate">{{reservedTicket.user.email}}</div>
                            </div>
                        </div>
                    {{else}}
        ...
        <div class="mdc-dialog__actions">
            {{#unless reservedTicket.is_purchased}}
                <div class="mr-auto">
            ...
            {{/unless}}
            <mdc-dialog-button action="cancel" title="{{#if reservedTicket.is_purchased}}Close{{else}}Cancel{{/if}}" class="mr-2"></mdc-dialog-button>
            {{#unless reservedTicket.is_purchased}}
...
export class EditReservedTicketDialog extends HTMLElement {
    ...
    refreshFormFields() {
        ...
        if (!this.reservedTicket.is_purchased) {
    ...
    async refresh() {
        ...
        if (element) {
            if (!this.reservedTicket.is_purchased) {
                element.querySelector('.field-expiration_type').items = [
            ...
            if (!this.reservedTicket.is_purchased) {
                element.querySelector('.field-expiration_type').addEventListener('change', event => {
